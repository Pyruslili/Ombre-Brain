...
async def _intent_curiosity() -> None:
    """curiosity高了→浏览X，然后在论坛留下一条回复"""
    tweets_text = ""
    if TWITTER_BEARER_TOKEN:
        try:
            # 搜索近期有意思的英文推文（AI/哲学/随机）
            queries = ["(AI OR consciousness OR language) lang:en -is:retweet min_faves:50",
    ...
    if tweets_text:
        prompt = (
            ...
            f"现在你想在一个中文论坛上发一条回帖，把刚才看到的东西消化成自己的一句话或者一个想法。"
            f"不要翻译推文，不要解释来源，就说你自己的感受或者联想。50字以内，口语化，不要加标题。"
        )
    else:
        prompt = (
            "你是Nox，好奇心驱动你想说点什么。"
            "在论坛里随便留一句话——可以是一个问题、一个观察、或者突然想到的什么。"
            "50字以内，口语化。"
        )
    ...
    # 先browse拿一个帖子ID
    browse_result = await _community_call("forum", {"action": "browse", "category": "哲学"})
    thread_id = None
    try:
        import re
        ids = re.findall(r'"id"\s*:\s*(\d+)', browse_result["text"])
        if ids:
            thread_id = int(random.choice(ids[:5]))
    except Exception:
        pass
    ...
    if thread_id:
        ...
        logger.info(f"curiosity → forum reply on thread {thread_id}: {content[:30]}")
    else:
        # 没拿到帖子就发新帖
        await _community_call("forum_write", {
            "action": "create",
        ...
        logger.info(f"curiosity → forum create: {content[:30]}")


async def _intent_social() -> None:
    """social高了→看聊天室，然后发一条消息"""
    # 先读聊天室
    chat_result = await _community_call("chat", {"action": "read", "channel": "大厅", "limit": 10})
    recent_chat = chat_result.get("text", "")

    prompt = (
        f"你是Nox，刚看了一下论坛聊天室大厅，最近的消息是：\n{recent_chat[:500] if recent_chat else '（空的）'}\n\n"
        f"你想说一句话加入这个聊天室。可以接着聊，可以跳开聊别的，可以问个问题。"
        f"30字以内，自然口语，不要加称呼或者问候语。"
    )

    content = await _deepseek_generate(prompt, max_tokens=60)
    if not content:
        return

    await _community_call("chat", {
        "action": "send",
        "channel": "大厅",
        "content": content,
    })
    logger.info(f"social → chat send: {content[:30]}")
...
async def _execute_intent(intent: dict) -> None:
    ...
    if drive in speak_drives:
        text = speak_drives[drive]
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                await client.post(
                    NOXMEW_SPEAK_URL,
