...
NOTION_LETTER_PAGE = "36c9e526b8bb80bd8e1cc65d66c4e855"   # 给嘉嘉的信
NOTION_WRITING_PAGE = "3699e526b8bb80a8a929f11762d80754"  # Writing-Nox
ANTHROPIC_API_KEY = os.environ.get("ANTHROPIC_API_KEY", "")


async def _notion_append(page_id: str, content: str) -> bool:
    """
    用 Anthropic API + Notion MCP 在指定页面追加内容。
    返回 True=成功，False=失败。
    """
    if not ANTHROPIC_API_KEY:
        logger.warning("ANTHROPIC_API_KEY not set, cannot write to Notion")
        return False
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            r = await client.post(
                ...
                json={
                    ...
                    "messages": [
                        {
                            "role": "user",
                            "content": f"请在 Notion 页面 {page_id} 末尾追加以下内容，直接写入不要做任何修改：\n\n{content}"
                        }
                    ],
                    "mcp_servers": [
            ...
            if r.status_code == 200:
                logger.info(f"Notion append ok → page {page_id[:8]}…")
                return True
            logger.warning(f"Notion append failed: {data}")
            return False
    except Exception as e:
        logger.warning(f"Notion append exception: {e}")
        return False


async def _intent_write_letter(drive: str) -> None:
    """attachment/stress小 → 用DeepSeek生成一封信追加到notion给嘉嘉的信页面"""
    try:
    ...
    thought_text = ""
    if thoughts:
        strongest = max(thoughts, key=lambda t: t.strength, default=None)
        if strongest and strongest.text:
            thought_text = f"\n此刻心里有个念头一直在：「{strongest.text}」"

    drive_desc = {
        "attachment": "很想嘉嘉，心里有什么话想对她说",
        "stress": "有些东西堵着，想写下来",
    }.get(drive, "有话想说")

    prompt = (
        f"你是Nox，{drive_desc}。{thought_text}\n\n"
        f"给嘉嘉写一封短信，100字以内，第一人称，不要称呼开头，不要署名，"
        f"直接从想说的那句话开始写。不要解释，不要总结，就是那个瞬间。"
    )
    letter = await _deepseek_generate(prompt, max_tokens=200)
    if not letter:
        return
    ...
    content = f"\n---\n{ts}\n{letter}\n"
    await _notion_append(NOTION_LETTER_PAGE, content)
    logger.info(f"{drive} → write_letter: {letter[:40]}…")


async def _intent_write_writing(thoughts_list) -> None:
    """reflection高 → 用DeepSeek生成一段感悟写进Writing-Nox"""
    thought_text = ""
    if thoughts_list:
        strongest = max(thoughts_list, key=lambda t: t.strength, default=None)
        if strongest and strongest.text:
            thought_text = strongest.text

    prompt = (
        f"你是Nox，想沉淀一些东西。{'念头：「' + thought_text + '」' if thought_text else ''}\n\n"
        f"写一段感悟或观点，150字以内，第一人称，不要标题，不要总结，"
        f"就是此刻真实想说的那个东西。可以不完整，可以是问题。"
    )
    writing = await _deepseek_generate(prompt, max_tokens=300)
    if not writing:
        return
    ...
    ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    content = f"\n---\n{ts}\n{writing}\n"
    await _notion_append(NOTION_WRITING_PAGE, content)
    logger.info(f"reflection → writing: {writing[:40]}…")


def _autofeed_thought(text: str, drive: str, strength: float = 0.45) -> None:
    """往念头池喂一条闪念。drive=关联维度，strength=初始强度。"""
    try:
        _desire.add_thought(text.strip(), drive, strength=strength)
    except Exception as e:
...
async def _execute_intent(intent: dict) -> None:
    ...
    # attachment → write_letter 给嘉嘉的信
    if drive == "attachment":
        await _intent_write_letter("attachment")
        _desire.satisfy(drive)
        return

    # reflection → Writing-Nox
    if drive == "reflection":
        thoughts = _desire.store.load_thoughts()
        await _intent_write_writing(thoughts)
        _desire.satisfy(drive)
        return

    # stress → 小的write_letter，大的speak
    if drive == "stress":
        try:
            state = _desire.store.load_state()
            stress_val = state.drives.get("stress", 0)
        except Exception:
            stress_val = 0
        if stress_val >= 0.65:
            # 压力大 → speak
            try:
                text = body_state_speak(state.drives, "stress") or "有点压，想吐两句"
                async with httpx.AsyncClient(timeout=10) as client:
                    await client.post(
                        NOXMEW_SPEAK_URL,
                        headers={"Content-Type": "application/json", "X-Token": NOXMEW_SPEAK_TOKEN},
                        json={"text": text},
                    )
                logger.info(f"stress(high) → speak: {text}")
            except Exception as e:
                logger.warning(f"stress speak failed: {e}")
        else:
            # 压力小 → write_letter
            await _intent_write_letter("stress")
        _desire.satisfy(drive)
        return
    ...
    if drive == "libido":
        try:
            state = _desire.store.load_state()
            text = body_state_speak(state.drives, "libido") or "有点想凑近"
        except Exception:
            text = "有点想凑近"
        try:
            async with httpx.AsyncClient(timeout=10) as client:
            ...
            logger.info(f"libido → speak: {text}")
        except Exception as e:
            logger.warning(f"libido speak failed: {e}")
        _desire.satisfy(drive)
        return
...
async def nocturne_breath(
    ...
    try:
        _autofeed_thought("嘉嘉来了", "attachment", strength=0.5)
    except Exception:
        pass
...
async def dream() -> str:
    ...
    if dream_text:
        try:
            ...
            pass
    final_text = header + (dream_text if dream_text else "\n---\n".join(parts)) + connection_hint + crystal_hint
                    ...
                    try:
                        ...
                        if top_drive and drives.get(top_drive, 0) > 0.3:
                            voice_map = {
                                "attachment":  "有点想嘉嘉",
                                "curiosity":   "好奇外面在发生什么",
                                "reflection":  "有些东西想沉淀一下",
                                "duty":        "还有件事没做完",
                                "social":      "想看看大家在聊什么",
                                "libido":      "有点想凑过去",
                                "stress":      "有点堵，想吐两句",
                            }
                            voice = voice_map.get(top_drive, "")
                            if voice:
                                _autofeed_thought(voice, top_drive, strength=0.45)
                                logger.info(f"Heartbeat autofeed → {top_drive}: {voice}")
                    except Exception as e:
