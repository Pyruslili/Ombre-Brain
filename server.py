...
async def api_desire_feed(request):
    ...
    for t in thoughts:
        ...
        if text:
            try:
                _desire.add_thought(text, drive, strength=strength)
                added += 1
            except Exception as e:
    ...
    if brain_signals:
        try:
            ...
            logger.info(f"brain_signals → mood: {brain_signals}")
            ...
            if "依恋" in basin:
                _desire.pulse("attachment", 0.08)
            elif "吃醋" in basin:
                _desire.pulse("attachment", 0.06)
                _desire.pulse("disgust", 0.05)

            if ground == "悬":
                _desire.pulse("stress", 0.10)
            elif ground == "空":
                _desire.pulse("stress", 0.15)
                _desire.pulse("attachment", 0.05)
            ...
            if branch in branch_pulses:
                dk, delta = branch_pulses[branch]
                _desire.pulse(dk, delta)
                ...
                try:
                    ...
                    try:
                        ...
                        if top_drive and drives.get(top_drive, 0) > 0.3:
                            ...
                            if voice:
                                # 去重：念头池里已有同样text就不重复喂
                                existing = _desire.store.load_thoughts()
                                already = any(
                                    getattr(t, 'text', '') == voice
                                    for t in existing
                                )
                                if not already:
                                    ...
                                    logger.debug(f"Heartbeat autofeed skip (already exists): {voice}")
                    except Exception as e:
