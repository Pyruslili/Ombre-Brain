...
九条 Drive 的 baseline 不同，raw value 不能直接当百分比比较。持久层继续保存 raw，展示和 Intent 使用 baseline-normalized activation：

`linear_pressure = clamp((raw - baseline) / (1 - baseline))`

`activation = sqrt(linear_pressure)`

`effective_activation = activation * (1 - local_fatigue)`

- baseline 永远显示为 `0%` 激活，raw 仍在 dashboard 详情和 API 中可见。
- 平方根响应用于把普通、明显、决定性事件拉开：普通约 `15-40%`，明确事件可越过 `50%`，极强事件才接近 `80%+`。
- Intent 统一比较 `effective_activation`，当前门槛 `0.55`，不再让 baseline 高的 Drive 天生更容易获胜。
- `effective_drives` 仍是 raw 乘 fatigue 的兼容诊断值；新增 `drive_activations / effective_activations` 才是展示和行动尺度。

Coupling 同样按真实变化量和 wall-clock tick scale 计算：`delta` 模式乘 source 的实际增量；`level` 模式和 damping 按相对 30 分钟 tick 的经过时间缩放，避免改心跳频率就改变一天内累计总量。
...
- `brain`: 底层特征和目标
- `evidence`: 证据摘录
- `thoughts`: 只有 Nox 自己的来源可写（manual / stir / 允许的 CLI feel）。`dialogue_residue` 与 **`dp_memory` 固定空**——记忆分析只染 Drive/Weather，不代写第一人称念头

Drive 记账采用 primary-focused 口径：

- primary 只记一次；对应 brain feature 用作主增量倍率，不再作为第二份同 Drive 增量相加。
- secondary 低于 `0.25` 不写长期 Drive。
- 非 primary brain feature 低于 `0.45` 或计算增量低于 `0.003` 时只参与 Chemistry / Weather，不写 Drive。
- 所有旁支增量合计不超过 primary 的 `65%`；因此 analyzer 填满字段也不会把一个事件扩散成九维浓汤。
